...
C  9100  FORMAT(1X,'Side',I2,' forced transition at x/c = ',F7.4,I5)
C       ELSE
C        WRITE(*,9200) IS,XOCTR(IS),ITRAN(IS)
C  9200  FORMAT(1X,'Side',I2,'  free  transition at x/c = ',F7.4,I5)
C       ENDIF
C
C---- next airfoil side
 2000 CONTINUE
...
C
     ...
     &        / (GAMM1*(1.0 - 0.5*UEDG(IBL,IS)**2*HSTINV))
          ...
          CALL DSLIM(DSW,THET(IBL,IS),MSQ,HKLIM)
...
C
      ...
      SUBROUTINE DSLIM(DSTR,THET,MSQ,HKLIM)
