...
C
   ...
   20 CONTINUE
      ...
      SUBROUTINE XLFIND(SLE,X,XP,S,N)
      DIMENSION X(*),XP(*),S(*)
C------------------------------------------------------
...
C       SUBROUTINE CLIS(X,XP,Y,YP,S,N)
C       DIMENSION X(*), XP(*), Y(*), YP(*), S(*)
C C-------------------------------------------------------------------
C C     Displays curvatures at panel nodes.
C C-------------------------------------------------------------------
C       PI = 4.0*ATAN(1.0)
C C
C       CMAX = 0.0
C       IMAX = 1
C C
C C---- go over each point, calculating curvature
C       WRITE(*,1050)
C       DO 30 I=1, N
C         IF(I.EQ.1) THEN
C          ARAD = ATAN2(-YP(I),-XP(I))
C         ELSE
C          ARAD = ATANC(-YP(I),-XP(I),ARAD)
C         ENDIF
C         ADEG = ARAD * 180.0/PI
C         CV = CURV(S(I),X,XP,Y,YP,S,N)
C         WRITE(*,1100) I, X(I), Y(I), S(I), ADEG, CV
C         IF(ABS(CV) .GT. ABS(CMAX)) THEN
C          CMAX = CV
C          IMAX = I
C         ENDIF
C    30 CONTINUE
...
C C
C  1050 FORMAT(
C      &    /'  i        x         y         s       theta        curv')
C CCC           120   0.12134  -0.10234  -0.30234   180.024      2025.322
C  1100 FORMAT(1X,I3, 3F10.5, F11.3, F12.3)
C  1200 FORMAT(/' Maximum curvature =', F14.3,
C      &        '   at  i,x,y,s  = ', I3, 3F9.4 )
C       END SUBROUTINE CLIS




C       SUBROUTINE CANG(X,Y,N,IPRINT, IMAX,AMAX)
C       DIMENSION X(*), Y(*)
C C-------------------------------------------------------------------
C C     IPRINT=2:   Displays all panel node corner angles
C C     IPRINT=1:   Displays max panel node corner angle
C C     IPRINT=0:   No display... just returns values
C C-------------------------------------------------------------------
C C
C       AMAX = 0.0
C       IMAX = 1
C C
C C---- go over each point, calculating corner angle
C       IF(IPRINT.EQ.2) WRITE(*,1050)
C       DO 30 I=2, N-1
C         DX1 = X(I) - X(I-1)
C         DY1 = Y(I) - Y(I-1)
C         DX2 = X(I) - X(I+1)
C         DY2 = Y(I) - Y(I+1)
C C
C C------ allow for doubled points
C         IF(DX1.EQ.0.0 .AND. DY1.EQ.0.0) THEN
C          DX1 = X(I) - X(I-2)
C          DY1 = Y(I) - Y(I-2)
C         ENDIF
C         IF(DX2.EQ.0.0 .AND. DY2.EQ.0.0) THEN
C          DX2 = X(I) - X(I+2)
C          DY2 = Y(I) - Y(I+2)
C         ENDIF
C C
C         CROSSP = (DX2*DY1 - DY2*DX1)
C      &         / SQRT((DX1**2 + DY1**2) * (DX2**2 + DY2**2))
C         ANGL = ASIN(CROSSP)*(180.0/3.1415926)
C         IF(IPRINT.EQ.2) WRITE(*,1100) I, X(I), Y(I), ANGL
C         IF(ABS(ANGL) .GT. ABS(AMAX)) THEN
C          AMAX = ANGL
C          IMAX = I
C         ENDIF
C    30 CONTINUE
C C
C       IF(IPRINT.GE.1) WRITE(*,1200) AMAX, IMAX, X(IMAX), Y(IMAX)
C C
C       RETURN
C C
C  1050 FORMAT(/'  i       x        y      angle')
C CCC             120   0.2134  -0.0234   25.322
C  1100 FORMAT(1X,I3, 2F9.4, F9.3)
C  1200 FORMAT(/' Maximum panel corner angle =', F7.3,
C      &        '   at  i,x,y  = ', I3, 2F9.4 )
C       END SUBROUTINE CANG
...
C------- Remove node I+1
         ...
         N = N - 1
         LCHANGE = .TRUE.
C         WRITE(*,*) 'SCHECK segment removed at ',I
         GO TO 10
        ENDIF
C
 20   CONTINUE
