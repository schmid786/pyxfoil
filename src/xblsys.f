...
C---- set KE thickness shape parameter from  H - H*  correlations
      IF(ITYP.EQ.1) THEN
       CALL HSL( HK2, HS2, HS2_HK2, HS2_RT2, HS2_M2 )
      ELSE
...
C----- laminar
       CALL CFL( HK2, RT2, CF2, CF2_HK2, CF2_RT2, CF2_M2 )
      ELSE
C----- turbulent
       CALL CFT( HK2, RT2, M2, CF2, CF2_HK2, CF2_RT2, CF2_M2 )
       CALL CFL( HK2, RT2, CF2L,CF2L_HK2,CF2L_RT2,CF2L_M2)
       IF(CF2L.GT.CF2) THEN
...
C---- midpoint skin friction coefficient  (zero in wake)
      IF(ITYP.EQ.3) THEN
       ...
       CFM_MS  = 0.
      ELSE IF(ITYP.EQ.1) THEN
       CALL CFL( HKA, RTA, CFM, CFM_HKA, CFM_RTA, CFM_MA )
      ELSE
       CALL CFT( HKA, RTA, MA, CFM, CFM_HKA, CFM_RTA, CFM_MA )
       CALL CFL( HKA, RTA, CFML,CFML_HKA,CFML_RTA,CFML_MA)
       IF(CFML.GT.CFM) THEN
...
C
      CALL HSL( HK, HS, HS_HK, HS_RT, HS_MSQ )
C
...
C
      ...
      SUBROUTINE HSL( HK, HS, HS_HK, HS_RT, HS_MSQ )
C
...
C
      ...
      SUBROUTINE CFL( HK, RT, CF, CF_HK, CF_RT, CF_MSQ )
C
